...
- (void) URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if ( task.state == NSURLSessionTaskStateCompleted ) {
        ...
        // Error
        if (error != nil){
            if ( obtask.attemptCount >= MAX_ATTEMPTS ) {
                [self handleCompleted:task marker:marker obtask:obtask error:error];
            } else {
                [[self transferTaskManager] queueForRetry:obtask];
                [self.delegate fileTransferRetrying:marker isUpload:obtask.typeUpload attemptCount:obtask.attemptCount withError:error];
            }
        // No error
        } else {
            ...
            // after we have finished moving the file.
            if (obtask.typeUpload){
                [self handleCompleted:task marker:marker obtask:obtask error:error];
            }
        }
...
- (void) handleCompleted:(NSURLSessionTask *)task marker:(NSString * )marker obtask:(OBFileTransferTask *)obtask error:(NSError *)error{
    [[self transferTaskManager] removeTransferTaskForNsTask:task];
    [self.delegate fileTransferCompleted:marker isUpload:obtask.typeUpload withError:error];
}
...
- (void) URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didSendBodyData:(int64_t)bytesSent totalBytesSent:(int64_t)totalBytesSent totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend
{
    ...
    if ( [self.delegate respondsToSelector:@selector(fileTransferProgress:isUpload:percent:)] ) {
        NSString *marker = [[self transferTaskManager] markerForNSTask:task];
        [self.delegate fileTransferProgress:marker isUpload:YES percent:percentDone];
    }
}
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)task didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite{
    ...
    if ( [self.delegate respondsToSelector:@selector(fileTransferProgress:isUpload:percent:)] ) {
        NSString *marker = [[self transferTaskManager] markerForNSTask:task];
        [self.delegate fileTransferProgress:marker isUpload:NO percent:percentDone];
    }
}
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location{
    ...
    if ( response.statusCode/100 == 2   ) {
        // Copy the file to our downloads location...
        NSError * error;
        ...
        // GARF assumes download is successful inspite of the possible errors in moving the file.
        OBFileTransferTask * obtask = [[self transferTaskManager] transferTaskForNSTask:downloadTask];
        [self handleCompleted:downloadTask marker:marker obtask:obtask error:nil];
    } else {

...
@interface ZZFriendManager ()

@property (nonatomic, strong) ZZFriendDomainModel* friendDomainModel;

@end

@implementation ZZFriendManager

- (void)updateWithDomainModel:(ZZFriendDomainModel *)friendModel
{
    self.friendDomainModel = friendModel;
}
...
- (NSString *)displayName
{
    ...
    if ([self firstNameIsUnique])
        d = self.friendDomainModel.firstName;
    else
        d = [NSString stringWithFormat:@"%@. %@", [self firstInitial], self.friendDomainModel.lastName];
...
{
    return self.friendDomainModel.videos;
}

- (BOOL)hasIncomingVideo
{
    return [self.friendDomainModel.videos count] > 0;
}

- (NSArray *)sortedIncomingVideos
{
    NSSortDescriptor *d = [[NSSortDescriptor alloc] initWithKey:@"videoId" ascending:YES];
    return [self.friendDomainModel.videos sortedArrayUsingDescriptors:@[d]];
}
...
{
    OB_INFO(@"generateThumbWithVideo: %@ vid:%@", self.friendDomainModel.firstName, video.videoId);
...
- (void)copyToLastThumbWithVideo:(TBMVideo *)video
{
    if ([video hasThumb])
    {
        ...
        if (error != nil)
            OB_ERROR(@"copyToLastThumbWithVideo: %@ vid:%@ %@", self.friendDomainModel.firstName, video.videoId, error);
    }
}
